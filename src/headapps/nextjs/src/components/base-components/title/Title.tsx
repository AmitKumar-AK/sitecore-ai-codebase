...
const ComponentContent = ({
  ...
  styles = "",
  children,
}: TitleComponentContentProps): JSX.Element => (
...
export const Default = ({ params, fields, page }: TitleProps): JSX.Element => {
  ...
  const datasourceField = getFieldValue(datasource?.field) as
    | TextField
    | undefined;
  const contextField: TextField = page?.layout?.sitecore?.route?.fields
    ?.Title as TextField;
